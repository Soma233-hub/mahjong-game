...
## 夜間セッション確認記録（2026-05-25）
- 全テスト通過確認: 485/485 ✅ (19 + 26 + 52 + 134 + 89 + 5 + 137 + 23)
- **Phase UI-2 / 2-A 実装完了（ツモアニメーション）**
  - `GameScene._animateDrawP0()` 追加（P0専用 300ms スライドIN）
  - ツモ牌以外を n 枚レイアウト座標に即時描画 → 完了時の手牌シフトなし
  - ツモ牌は (760, 395) テーブル中央右から `Power2.easeOut` で手牌末尾へ
  - アニメ完了後に `_renderHand(0)` + `_showPlayer0Actions()` を呼ぶ
  - P1/P2/P3（AI）は即時描画維持（AI は 300ms より早く打牌→二重描画を防止）
  - `_onDraw`: playerIndex===0 → `_animateDrawP0()`, それ以外 → `_renderHand()` に分岐
  - `kanDraw` イベントも同じ `_onDraw` を経由するため嶺上ツモも対象
