...
## 平日夜セッション確認記録（2026-06-11）
- 全テスト通過確認: 493/493 ✅ (19 + 26 + 52 + 134 + 89 + 5 + 137 + 31)
- **Phase UI-8 / 8-B 実装完了（X線モード）**
  - GameScene.js: `_xray` フラグ追加（初期値 false）
  - `_buildStaticUI()` に「X線」ボタン追加（x=80, y=668）
  - `_toggleXray()` メソッド追加（フラグ反転 + P1/P2/P3 手牌即時再描画）
  - `_renderHandTop/Right/Left`: `back: true` → `back: !this._xray` に変更
  - ON 時: 緑ボタン + 他家手牌を実牌テクスチャで表示
  - OFF 時: 暗褐色ボタン + 裏牌テクスチャで表示（通常状態）
