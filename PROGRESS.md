...
| 4-E | ブラウザ実機テスト・レイアウト最終調整 | ⬜ |

---

## 土曜午後セッション確認記録（2026-05-31）
- 全テスト通過確認: 485/485 ✅ (19 + 26 + 52 + 134 + 89 + 5 + 137 + 23)
- **Phase UI-4 / 4-A〜4-D 実装完了**
  - 【4-A】テンパイ候補ハイライト
    - `_computeTenpaiCandidates(player)`: P0 がツモ時に14枚手牌の各打牌候補を仮除去し `isTenpai()` チェック
    - テンパイになる打牌候補インデックスを `_tenpaiCandidates[]` に格納
    - `_drawTile()` に `tenpai` オプション追加（薄緑 tint 0x88ff88、選択 tint が優先）
    - `_renderHand0()` で `_tenpaiCandidates.includes(idx)` かつ非リーチ時に適用
    - `_onDraw` 直前に計算、`_onDiscard` で P0 打牌後にクリア
  - 【4-B】点数移動フロートテキスト
    - `_showScoreFloat(playerIndex, delta)`: 各プレイヤー位置からテーブル中央方向へ 1400ms スライド+フェードアウト
    - P0 (下→上) / P1 (右→左) / P2 (上→下) / P3 (左→右) 方向に流れる
    - `_prevScores = Array(4).fill(25000)` で初期化、`_onNextRound` で次局前に保存
    - `_showRoundEndPanel` 内で `g.players[i].score - _prevScores[i]` の差分を表示
  - 【4-C】プレイヤー風牌アイコン常時表示
    - スコアバーの各プレイヤー名に席風文字（東南西北）を前付け
    - `windChars[(i - g.dealerIndex + 4) % 4]` で席風を決定（親=東、その左=南…）
    - `_updateInfoTexts()` 呼び出し毎に自動更新（親交代にも追従）
  - 【4-D】スコアバーリアルタイムアニメーション
    - `_animateScoreChange(playerIndex, delta)`: 得点増加→金色(0xffdd44)、減少→赤(0xff6666) tint
    - scale 1→1.3→1 の yoyo tween（200ms）で視覚的フィードバック
    - `_showRoundEndPanel` 内で 4-B の floatと同タイミングで発火

## 次回作業内容
- **現在のフォーカス**: Phase UI-4 残1件（4-E: ブラウザ実機テスト・レイアウト最終調整）
- Phase UI-4 完了後: 最終完成
