...
## 夜セッション確認記録（2026-05-08）
- 全テスト通過確認: 323/323 ✅ (14 + 24 + 52 + 85 + 88 + 5 + 55)
- バグ修正: 天和・地和が Game._calculateWin で常に false にハードコードされていた問題
  - _meldOccurred フラグを追加（_startRound でリセット、各副露関数でセット）
  - isTenhou: turn===1, dealer===winner, !rinshan, !meldOccurred
  - isChiihou: 子の最初のツモターン（(winner-dealer+4)%4+1）, !rinshan, !meldOccurred
  - tests/test-edge-cases.js に天和・地和テスト18件追加（37→55件）
- TDDサイクル: Red(4件失敗) → 実装 → Green(55件全通過)
...
## 完了タスク（第6週・続き）
- [x] 天和・地和 Game._calculateWin への接続バグ修正（常にfalseだった問題）
- [x] _meldOccurred フラグ追加（局開始リセット + 全副露関数でセット）
- [x] tests/test-edge-cases.js に天和・地和テスト18件追加（323テスト全通過）
