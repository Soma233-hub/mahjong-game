...
### Phase UI-10: プレイ支援 & 観戦 UX 強化（第15週〜）
**目的**: フリテン視覚化・観戦モード快適化・局情報充実

| 工程 | 内容 | 対象ファイル | 状態 |
|------|------|------------|------|
| 10-A | フリテン警告表示（テンパイ+フリテン時に赤/オレンジで「フリテン: 待ち牌」表示） | GameScene.js | ✅ 完了 |
| 10-B | 観戦モード自動進行（局終了パネルを3秒後に自動で次局へ、カウントダウン表示） | GameScene.js | ✅ 完了 |
| 10-C | 残り局数表示（局情報行に「残N局」を追加） | GameScene.js | ✅ 完了 |
| 10-D | ロン放銃者表示（局終了パネルに「○○からのロン」を追加） | GameScene.js | ✅ 完了 |

---

## 平日夜セッション確認記録（2026-06-15）
- 全テスト通過確認: 493/493 ✅ (19 + 26 + 52 + 134 + 89 + 5 + 137 + 31)
- **Phase UI-10 新規策定・全4タスク完了（プレイ支援 & 観戦 UX 強化）**
  - 【10-A】フリテン警告表示
    - `_updateRiichiWaitHint()` に `isFuriten` / `isTemporaryFuriten` チェックを追加
    - テンパイ + 一時フリテン: 「一時フリテン: 1m 4m」をオレンジ (#ffaa44) で表示
    - テンパイ + 永続フリテン: 「フリテン: 1m 4m」を赤 (#ff4444) で表示
    - テンパイ + フリテンなし: 従来通り「テンパイ: 1m 4m」を緑表示
  - 【10-B】観戦モード自動進行（3秒カウントダウン）
    - `_showRoundEndPanel()` で `_allAI` 時に `this.time.addEvent()` で3秒タイマーを設定
    - 「3秒後に自動進行 → 2秒 → 1秒」カウントダウンテキストを次局ボタン下に表示
    - カウントダウン終了または手動クリック時に `advance()` でパネルを破棄し次局へ
  - 【10-C】残り局数表示
    - `_updateInfoTexts()` の局情報テキストに `残${_maxRounds - round}局` を追加
    - 例: 「東1局 0本場 残4局」「南3局 1本場 残2局」
  - 【10-D】ロン放銃者表示
    - `_onRoundEnd` の destructuring に `discarderIndex` を追加
    - `_showRoundEndPanel()` のRON行に「（右からのロン）」などの放銃者を表示
    - `discarderIndex` を `panelData` に格納して `_showRoundEndPanel` に渡す

## 次回作業内容
- **現在のフォーカス**: Phase UI-10 全完了。次フェーズ（UI-11）の策定待ち
